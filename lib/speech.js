...
const MESSAGES = {
  ...
  scan_prompt: {
    english: 'Scan the QR code for {product}.',
    tamil: '{product} க்கான QR குறியீட்டை ஸ்கேன் செய்யவும்.',
    malayalam: '{product} എന്നതിന്റെ QR കോഡ് സ്കാൻ ചെയ്യുക.',
    hindi: '{product} के लिए QR कोड स्कैन करें।',
  },
  tap_to_scan: {
    english: 'Pack {quantity} {unit} of {product}. Tap Scan QR when ready.',
    tamil: '{quantity} {unit} {product} பேக் செய்யவும். தயாரானால் Scan QR அழுத்தவும்.',
    malayalam: '{quantity} {unit} {product} പാക്ക് ചെയ്യുക. തയ്യാറാകുമ്പോൾ Scan QR ടാപ്പ് ചെയ്യുക.',
    hindi: '{quantity} {unit} {product} पैक करें। तैयार होने पर Scan QR दबाएं।',
  },
  select_item: {
    english: 'Tap an item below to start packing.',
    tamil: 'பேக்கிங் தொடங்க கீழே ஒரு பொருளைத் தட்டவும்.',
    malayalam: 'പാക്കിംഗ് ആരംഭിക്കാൻ താഴെ ഒരു ഇനം ടാപ്പ് ചെയ്യുക.',
    hindi: 'पैकिंग शुरू करने के लिए नीचे एक आइटम टैप करें।',
  },
};
...
export function syncWorkerLangFromProfile(preferredLang) {
  if (!preferredLang || typeof window === 'undefined') return;
  setWorkerLang(preferredLang);
}
...
export function extractProductIdFromScan(value) {
  ...
  // Match /p/PRODUCT_ID in URLs (case-insensitive — do not uppercase before this)
  const urlMatch = trimmed.match(/\/p\/([^/?#\s]+)/i);
  if (urlMatch?.[1]) {
