...
export function splitWorkerOrders(orders, workerId) {
  ...
  const active = mine.filter((o) => {
    if (ACTIVE_WORKER_STATUSES.includes(o.status)) return true;
    if (o.status === 'PACKED' && o.order_items && !isOrderFullyPacked(o.order_items)) return true;
    return false;
  });
  const completed = mine.filter(
    (o) => o.status === 'PACKED' && (!o.order_items || isOrderFullyPacked(o.order_items))
  );
...
export function canWorkerOpenOrder(order, workerId) {
  if (order.assigned_worker_id !== workerId) return false;
  if (ACTIVE_WORKER_STATUSES.includes(order.status)) return true;
  if (order.status === 'PACKED' && order.order_items && !isOrderFullyPacked(order.order_items)) {
    return true;
  }
  return false;
}
