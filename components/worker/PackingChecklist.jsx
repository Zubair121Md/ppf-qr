...
import { IconLeaf, IconCheck } from '@/components/ui/Icons';
      ...
      {items.map((item) => {
        ...
        return (
          <button
            ...
            `}
          >
            <div className="w-16 h-16 bg-surface-muted rounded-xl overflow-hidden flex-shrink-0 relative flex items-center justify-center">
              {product?.image_url ? (
                <Image src={product.image_url} alt={name} fill className="object-cover" sizes="64px" />
              ) : (
                <IconLeaf className="w-8 h-8 text-gray-300" />
              )}
            </div>
            ...
            <div className="flex-1 text-left min-w-0">
              ...
              <p className="text-xs font-mono text-gray-400 mt-0.5">{item.product_id}</p>
            </div>
